...
def test_register(client, app):
    ...
    assert b"""<li id="success" name="success">success</li>""" in response.data
    ...
    response = client.post("/register", data={"uname": "a", "pword": "a", "2fa": "12345"})
    assert b"""<li id="success" name="success">Failure - Unable to register. Please try again.</p>""" in response.data

    response = client.post("/register", data={"pword": "a", "2fa": "12345"})
    assert b"""<li id="success" name="success">Failure - Unable to register. Please try again.</p>""" in response.data

    response = client.post("/register", data={"uname": "a", "2fa": "12345"})
    assert b"""<li id="success" name="success">Failure - Unable to register. Password cannot be null.</p>""" in response.data
